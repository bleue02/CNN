...
# polling: average
class Image_16x16_5x5_max(nn.Module):
    def __init__(self, in_channels=3, num_classes=10):
        super(Image_16x16_5x5_max, self).__init__()
    ...
    def conv_layers(self, x):
        x = F.relu(self.bn_conv1(self.conv1(x))) # (16 -5 + 2 x 1) /1 + 1 -> 14
        x = F.relu(self.bn_conv2(self.conv2(x))) # (14 -5 + 2 x 1) /1 + 1 -> 12
        x = self.pool(x) # 12 / 2 -> 6
        x = self.dropout_conv(x)

        x = F.relu(self.bn_conv3(self.conv3(x))) # (6 -5 + 2 x 1) /1 + 1 -> 4
        x = F.relu(self.bn_conv4(self.conv4(x))) # (4 -5 + 2 x 1) /1 + 1 -> 2
        x = self.pool(x) # 2 / 1 -> 1
        x = self.dropout_conv(x)
        return x
...
if __name__ == '__main__':
    ...
    model = Image_16x16_5x5_max().to(device)
    summary(model, (3, 16, 16))


class Image_16x16_5x5_avg(nn.Module):
    def __init__(self, in_channels=3, num_classes=10):
        super(Image_16x16_5x5_avg, self).__init__()
        # Convolutional layers
        self.conv1 = nn.Conv2d(in_channels, 32, 5, padding=1)
        self.bn_conv1 = nn.BatchNorm2d(32)
...
if __name__ == '__main__':
    ...
    model = Image_16x16_5x5_avg().to(device)
    summary(model, (3, 16, 16))
