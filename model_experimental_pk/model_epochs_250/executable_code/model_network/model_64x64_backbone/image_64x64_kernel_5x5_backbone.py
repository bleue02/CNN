...
# polling: average
class Image_64x64_5x5_max(nn.Module):
    def __init__(self, in_channels=3, num_classes=10):
        super(Image_64x64_5x5_max, self).__init__()
    ...
    def conv_layers(self, x):
        x = F.relu(self.bn_conv1(self.conv1(x))) # (64 -5 + 2 x 2) /1 + 1 -> 64
        x = F.relu(self.bn_conv2(self.conv2(x))) # (64 -5 + 2 x 2) /1 + 1 -> 64
        x = self.pool(x) # 64 / 2 -> 32
        x = self.dropout_conv(x)

        x = F.relu(self.bn_conv3(self.conv3(x))) # (32 -5 + 2 x 2) /1 + 1 -> 32
        x = F.relu(self.bn_conv4(self.conv4(x))) # (32 -5 + 2 x 2) /1 + 1 -> 32
        x = F.relu(self.bn_conv5(self.conv5(x))) # (32 -5 + 2 x 2) /1 + 1 -> 32
...
if __name__ == '__main__':
    ...
    model = Image_64x64_5x5_max().to(device)
    summary(model, (3, 64, 64))


class Image_64x64_5x5_avg(nn.Module):
    def __init__(self, in_channels=3, num_classes=10):
        super(Image_64x64_5x5_avg, self).__init__()
        # Convolutional layers
        self.conv1 = nn.Conv2d(in_channels, 64, 5, padding=2)
        self.bn_conv1 = nn.BatchNorm2d(64)
...
if __name__ == '__main__':
    ...
    model = Image_64x64_5x5_avg().to(device)
    summary(model, (3, 64, 64))
