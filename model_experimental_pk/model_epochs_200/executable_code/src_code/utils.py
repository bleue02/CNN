...
class Utils(metaclass=SingletonMeta):
    def __init__(self, dir='./data', train=True, download=False, transform=None):
        self.dir = dir
        self.train = train
    ...
    def get_data_loaders(self, batch_size=64, num_workers=4, input_size=(32, 32)): # defualt size value없으면 32x32 고정값 할당
        ...
        partial_train_set = torch.utils.data.Subset(full_train_set, subset_indices)

        torch.manual_seed(42)
        train_size = 7000
        val_size = 1500
        test_size = 1500
        train_set, val_set, test_set = random_split(partial_train_set, [train_size, val_size, test_size])
...
def load_model(net, base_dir='C:/Users/jdah5454/PycharmProjects/task_cifar10', filename='cifar10.pth', device='cuda'):
    save_dir = os.path.join(base_dir, 'pth')
    save_path = os.path.join(save_dir, filename)

    if not os.path.exists(save_path):
        logging.error(f"모델 파일이 존재하지 않습니다: {save_path}")
        return None

    state_dict = torch.load(save_path, map_location=torch.device(device))
    net.load_state_dict(state_dict)

    # DataParallel로 래핑된 모델인지 확인 후, 래핑
    if torch.cuda.device_count() > 1: # 분산 학습.
...
def save_history_to_csv(
        history, base_dir='C:/Users/jdah5454/PycharmProjects/task_cifar10', filename='training_history.csv'):
